...
//    generate the rotation matrix from the quaternion attitude, representing the orientation of the drone in the inertial frame relative to the body frame.
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
    ...
    V3F b_a = V3F(R13, R23, 0.f); // current lateral accelerations in the body frame.
    V3F b_c = V3F(accelCmd.x / -c, accelCmd.y / -c, 0.f); // commanded accelerations in the body frame
    b_c.constrain(-maxTiltAngle, maxTiltAngle);

    V3F b_c_dot = kpBank * (b_c - b_a);

    pqrCmd.x = (R21 * b_c_dot.x - R11 * b_c_dot.y) / R33;
    pqrCmd.y = (R22 * b_c_dot.x - R12 * b_c_dot.y) / R33;
    pqrCmd.z = 0.f; // set to zero because yaw control is handled separately.
  /////////////////////////////// END STUDENT CODE ////////////////////////////
